...
class Kohana_Session_Database extends Session {
	...
	{
		$user_id = 0;
        if ( ($userdata = Session::instance('database')->get('user')) !== FALSE) {
        ...
        if ($this->_update_id === NULL)
		{
			...
			$query = DB::insert($this->_table, $this->_columns)
				->values(array(':new_id', ':user_id', ':active', ':contents'));
		}
		else
		{
			// Update the row
			$query = DB::update($this->_table)
				->value('user_id', ':user_id') // NEW CHANGE
		...
		$query
			...
			->param(':user_id',  $user_id)// NEW CHANGE
